...
public class Chapter4Ex8 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < gs.length; i++) { // 모든 반 까지 출력
			int cnt = lengths[i];
			//★★★★★★★★★★★★★
			for (int j = 0; j < cnt-1; j++) {// 각 반의 학생 수만큼 반복
		...
		for (int j = 0; j < gs.length-1; j++) // gs라는 2차원 배열의 세로길이 = 반의 개수 만큼 반복을하구요
			for (int i = 0; i < gs.length - 1-j; i++) {// 그 안에서 반의 개수-1만큼 내림차순으로 정렬을 했습니다.
				if (gs[i][0] < gs[i + 1][0]) {// 이때 값의 비교는 각 반의 1등을 비교할 수 있도록 gs[i][0]과 gs[i+1][0]을 비교했습니다.
					int[] temp = gs[i];
					gs[i] = gs[i + 1];
